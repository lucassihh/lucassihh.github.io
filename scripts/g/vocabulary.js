...
// Icons
const ICON_PLAY = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-6 text-orange-400 group-hover:scale-110 transition-all duration-300"><path d="M3 14h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-7a9 9 0 0 1 18 0v7a2 2 0 0 1-2 2h-1a2 2 0 0 1-2-2v-3a2 2 0 0 1 2-2h3"/></svg>`;

const ICON_PAUSE = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="size-10 p-2 bg-orange-400 text-white rounded-full hover:scale-110 transition-all duration-300 shadow-sm animate-bounce"><rect x="14" y="3" width="5" height="18" rx="1"/><rect x="5" y="3" width="5" height="18" rx="1"/></svg>`;
...
function stopCurrentAudio() {
  ...
  if (currentButton) {
    ...
    // Remove bg-orange-100 and Reset text color
    const parentWrapper = currentButton.closest(".group");
    if (parentWrapper) {
      parentWrapper.classList.remove("bg-orange-100");

      const mainTextEl = parentWrapper.querySelector(".vocab-main-text");
      if (mainTextEl) {
        mainTextEl.classList.remove("text-orange-300");
        // mainTextEl.classList.add("text-primary");
      }
    }
...
function createPlayButton(audioSrc) {
  ...
  playButton.addEventListener("click", (e) => {
    ...
    const mainTextEl = parentWrapper
      ? parentWrapper.querySelector(".vocab-main-text")
      : null;
    ...
    // Set bg-orange-100 and text color if Playing
    if (parentWrapper) {
      parentWrapper.classList.add("bg-orange-100");
      if (mainTextEl) {
        mainTextEl.classList.remove("text-primary");
        mainTextEl.classList.add("text-orange-300");
      }
    }
...
function createVocabItemElement(item, fromLang, toLang) {
  ...
  wrapper.className =
    "flex items-center justify-between p-4 rounded-[1.5rem] bg-card group transition-all duration-300 cursor-pointer";
  ...
  // hintText.className = "text-xs text-muted flex items-center gap-1";
  // hintText.innerHTML = '<span class="size-2 rounded-full bg-orange-400"></span>Audio';
  ...
  // Main Text
  let mainText = document.createElement("h4");
  mainText.className =
    "font-bold text-lg text-primary capitalize vocab-main-text transition-colors duration-300";
...
export async function renderVocabulario({
  ...
  const checks = audioItems.map(async (card) => {
    ...
    if (exists) {
      ...
      // Opcional: card.addEventListener("click", () => playBtn.click());
    } else {
