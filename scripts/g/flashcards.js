    ...
    <div id="flashcards-modal" class="modal fixed inset-0 flex items-center justify-center z-40 hidden px-4">
      ...
      <div class="modal-dialog w-full h-full max-w-4xl relative flex flex-col transition-all duration-300 p-4 md:p-10">
        ...
        <div class="flex items-center justify-between gap-4 py-4 md:py-8">
          <button id="flashcard-play" class="flex items-center justify-center text-black/70 hover:text-black hover:scale-105 transition-all duration-300">
            ${ICONS.play}
          </button>
          <div id="flashcard-progress" class="text-lg font-black text-black/70"></div>
          <button data-modal-close class="flex items-center justify-center text-black/70 hover:text-black hover:scale-105 transition-all duration-300">
            ${ICONS.close}
          </button>
        </div>

        <!-- Main Design -->
        <div id="flashcard-scene" class="w-full h-[33rem] group mt-4" style="perspective: 1200px;">
          <div id="flashcard-inner" class="relative w-full h-full transition-transform duration-700 rounded-xl" style="transform-style: preserve-3d;">
            <div class="absolute inset-0 w-full h-full flex items-center justify-center p-12 bg-gray-100 rounded-xl" style="backface-visibility: hidden;">
              <span id="flashcard-front-text" class="text-black text-3xl md:text-5xl font-black text-center tracking-tighter"></span>
            </div>
            
            <div class="absolute inset-0 w-full h-full flex items-center justify-center p-12 bg-orange-400 rounded-xl" style="backface-visibility: hidden; transform: rotateY(180deg);">
              <span id="flashcard-back-text" class="text-white text-3xl md:text-5xl font-black text-center tracking-tighter"></span>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center justify-between gap-4 mt-6">
          <div class="flex gap-2">
            <button id="flashcard-prev" class="flex items-center justify-center p-1 text-black/70 hover:text-white hover:bg-orange-400 hover:scale-110 rounded-full transition-all duration-300">
              ${ICONS.prev}
            </button>
            <button id="flashcard-next" class="flex items-center justify-center p-1 text-black/70 hover:text-white hover:bg-orange-400 hover:scale-110 rounded-full transition-all duration-300">
              ${ICONS.next}
            </button>
          </div>
          <button id="flashcard-flip" class="flex items-center justify-center text-black/70 hover:text-white hover:bg-orange-400 hover:scale-110 rounded-full p-2 transition-all duration-300">
            ${ICONS.flip}
          </button>
        </div>
