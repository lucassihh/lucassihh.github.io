  ...
  playIcons.forEach((icon) => {
    icon.addEventListener("click", (e) => {
      ...
      if ("speechSynthesis" in window) {
        ...
        console.warn(
          "The Speech Synthesis API is not supported in this browser",
        );
      }
    });
  });
...
export async function initializeModuleTable(
  ...
  if (!container) {
    console.warn(
      `Destination div with ID '${moduleName}' not found. Skipping`,
    );
    return;
  }

  // Loading message
  container.innerHTML = `<p class="p-4 text-muted-foreground">Loading ${moduleName}...</p>`;

  try {
    const response = await fetch(jsonPath);
    if (!response.ok) {
      throw new Error(`Error loading ${jsonPath}: ${response.statusText}`);
    }
    ...
    console.error(`Error initializing the table of ${moduleName}:`, error);
    container.innerHTML = `<p class="p-4 text-red-500">The content of could not be loaded ${moduleName}: ${error.message}</p>`;
  }
}
